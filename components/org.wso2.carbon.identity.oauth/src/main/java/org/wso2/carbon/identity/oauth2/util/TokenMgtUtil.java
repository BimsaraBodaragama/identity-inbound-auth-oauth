    ...
    public static String getSigningTenantDomain(JWTClaimsSet claimsSet, AuthenticatedUser authenticatedUser)
        ...
        if (isJWTSignedWithSPKey) {
            try {
                ...
                /*
                 * Check if the OAuth application is a fragment application. Based on that, define the tenant that
                 * signed the JWT. In this case, the signing tenant should be extracted from application OIDC
                 * configurations. If that is not available, the root organization will be used as the signing tenant
                 * domain.
                 */
                ...
                throw new IdentityOAuth2Exception("Error while retrieving organization details for tenant domain: "
                        + consumerKey, e);
            }
        } else {
            if (LOG.isDebugEnabled()) {
